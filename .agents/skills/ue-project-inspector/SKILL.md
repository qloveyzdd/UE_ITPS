...
- `source-function.v1` returns every definition matching one selected function name. Results are grouped under `matches`; each match retains its owner, signature, `function_id`, compact declaration-definition relation, and one source-ordered `external_symbols` array. Every symbol has `kind`, `spelling`, and `evidence`; member calls and qualified callback targets also retain `owner_type` when locally derivable. Kinds classify candidate nodes rather than call, read, write, or ownership relations. Types are normalized from locally visible declarations, global variables use visible file declarations plus conservative `G*` spelling evidence, and bare calls are classified as free functions only after excluding parameters, locals, member variables, constructors, and macros. A scope-qualified call becomes `member_call` only when the selected source unit confirms the receiver as a type; otherwise the full expression remains `unknown`. Function addresses become callback targets only under recognized callback APIs. No included source, inheritance, overload, macro expansion, or called function is followed.
