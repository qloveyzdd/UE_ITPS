...
- Every source tool requires one explicitly selected `.h/.hpp/.cpp/.cc` and automatically derives opposite-kind, same-name companion candidates. Derivation checks the selected file's directory and conventional module `Private` to `Public` or `Classes` mirror paths in either direction. A `.cpp/.cc` entry searches `.h/.hpp`, while a `.h/.hpp` entry searches `.cpp/.cc`. Only one candidate is scanned as the companion; zero or multiple candidates leave the corresponding `source_unit` field null, with multiple candidates reported in validation.
...
- `source-types.v1` reports separate `classes`, `structs`, `enums`, `interface_candidates`, `global_variables`, and `free_functions` arrays. Class, struct, and enum items distinguish `declaration` from `definition`, expose their lexical `namespace`, and include that namespace in `qualified_name`; compound types retain lexical class/struct `owner`. Global variables and free functions expose `namespace`, `qualified_name`, `role`, and locally observed `linkage` (`internal` or `external`); free functions also retain a compact `signature`. A scope-qualified variable or function definition is classified as a namespace symbol only when its qualifier resolves to a namespace observed in the selected source pair; other qualified definitions remain member-shaped. Class and struct definitions own source-ordered `member_anchors`; each member anchor is a `variable` or `function`. Interface candidates retain the local reasons that caused the classification and are not UHT-confirmed interfaces. Free functions exclude recognized macro-like declarations. Type, declaration, and member evidence uses `unit` (`cpp` or `header`) plus source lines. The selected source and its uniquely derived companion are reported independently: declarations and definitions are not merged across files, and no project-level symbol IDs are created. Type and reflection macros are attached by lexical adjacency; this is not UHT semantic analysis or a complete C++ symbol table.
- `source-function.v1` returns every definition matching one selected function name. Results are grouped under `matches`; each match retains its lexical `namespace`, `qualified_name`, owner, signature, `function_id`, compact declaration-definition relation, and one source-ordered `external_symbols` array. Method owners retain the full lexical enclosing class chain without repeating the namespace. Namespace and the complete owner chain participate in the source-pair-local identity, declaration-definition matching, and `function_id`; this compact ID is not project-level or stable across source changes. Every symbol has `kind`, `spelling`, and `evidence`; member calls and qualified callback targets also retain `owner_type` when locally derivable. Kinds classify candidate nodes rather than call, read, write, or ownership relations. Types are normalized from locally visible declarations, and global variables use visible file declarations plus conservative `G*` spelling evidence. Namespace-qualified global references preserve their full source spelling and require either a matching fully qualified local declaration or conservative `G*` spelling evidence; `.`, `->`, confirmed-type `::`, unresolved qualifiers, and other unmatched qualified accesses remain `unknown`. A bare call becomes `member_call` only for a matching current-class method, or `free_function` only for a matching visible declaration in the selected source pair; otherwise the full expression remains `unknown`. A scope-qualified call becomes `member_call` when its receiver is a confirmed type, or `free_function` when its qualifier is an observed Namespace with a matching declaration; otherwise the full expression remains `unknown`. Function-address callable matching resolves owner spellings through the current lexical Namespace; addresses become callback targets only under recognized callback APIs, and `owner_type` is present only for confirmed type qualifiers. No included source, inheritance, overload, macro expansion, or called function is followed.
