...
- `source-types.v1` reports separate `classes`, `structs`, `enums`, `interface_candidates`, `global_variables`, and `free_functions` arrays. Class, struct, and enum items distinguish `declaration` from `definition`; compound types retain lexical `owner` and `qualified_name` for class/struct nesting. Class and struct definitions own source-ordered `member_anchors`; each member anchor is a `variable` or `function`. Interface candidates retain the local reasons that caused the classification and are not UHT-confirmed interfaces. Free functions distinguish declarations from definitions and exclude recognized macro-like declarations. Type, declaration, and member evidence uses `unit` (`cpp` or `header`) plus source lines. Type and reflection macros are attached by lexical adjacency; this is not UHT semantic analysis or a complete C++ symbol table.
