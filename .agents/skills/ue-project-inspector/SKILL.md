...
- `source-function.v2` returns every definition matching one selected function name. Results are grouped under `matches`; each match retains its owner, signature, `function_id`, compact declaration-definition relation, and one source-ordered `external_symbols` array. Every symbol has `kind`, `spelling`, and `evidence`; member calls and qualified callback targets also retain `owner_type` when locally derivable. Kinds classify candidate nodes rather than call, read, write, or ownership relations. Types are normalized from locally visible declarations, global variables use visible file declarations plus conservative `G*` spelling evidence, bare calls are classified as free functions after constructor and macro exclusions, and unresolved member receivers remain `unknown`. Function addresses become callback targets only under recognized callback APIs. No included source, inheritance, overload, macro expansion, or called function is followed.
