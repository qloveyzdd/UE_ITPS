...
python sourcetools/ue_list_cxx_includes.py --source <source> --compile-database <db>
python sourcetools/ue_list_cxx_types.py --source <source> --compile-database <db>
python sourcetools/ue_inspect_cxx_function.py --source <source> --function <name> --compile-database <db>
```
...
- `source-types.v1` reports separate `classes`, `structs`, `enums`, `interface_candidates`, `global_variables`, and `free_functions` arrays. Clang is authoritative for declaration/definition presence, qualified identity, bases, fields, functions, variable linkage, and source locations. UE reflection macros and interface-candidate reasons remain local source projections and are not UHT conclusions. The selected source and its uniquely derived companion are reported independently, and no project-level symbol IDs are created.
- `source-function.v1` returns every Clang-confirmed definition matching one selected function name. Results retain a compact source-pair `function_id`, declaration-definition relation, and source-ordered `external_symbols`. Call targets, receiver owners, type references, globals, function addresses, and callback targets come from the active Clang translation unit; UE delegate publish/subscribe operations remain a focused domain projection over the selected function source. Kinds are navigation categories rather than read/write or ownership semantics, and called function bodies are not followed.
...
- Clang may read transitive headers required by the selected compile command, but the public result emits semantic facts only from the selected source pair. The model must explicitly select another source unit for deeper inspection.
