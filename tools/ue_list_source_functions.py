#!/usr/bin/env python3
"""List callable signature facts from one selected C++ source unit."""

from pathlib import Path

from ue_project_tools.common import cli_error_document, cli_parser, json_text
from ue_project_tools.source_unit import list_source_functions


SCHEMA_VERSION = "ue-itps.source-functions.v1"
RESPONSIBILITY = (
    "Index callable signatures and conservative declaration-definition relations."
)


def main() -> int:
    parser = cli_parser(
        "列出一个显式选择的 C++ 源码单元中的函数索引事实。",
        "List callable index facts from one explicitly selected C++ source unit.",
    )
