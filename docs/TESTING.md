...
| `test_scope_analysis` | 覆盖与输入指纹、未知原因、跨文件候选、重载/条件定义、遮蔽与内部声明隔离、嵌套调用、目录复用一致性 |
...
2026-09-16：Scope 扩展验证覆盖当前全部 **175 项 SourceTools 用例**（核心 144 项、Lyra 31 项）。整组运行时的 173 项中，唯一错误是旧地图测试未注册新增 Schema；修复后该项与装备链 5 项复测通过，最终核心 144 项全部通过。总数包含期间补充的 2 项用例，来自整组运行与定向复测，不是一次 175 项命令的结果。Lyra 回归包含 707 文件 AST 对照。

5 份最终 JSON 产物通过 Schema 校验，27 份说明文件的本地链接检查通过，`git diff --check` 通过。旧组件结果保留其实际执行日期，执行次数不作为永久能力指标。

装备链定向扫描：5 组、10 个解析文件、13 个类型、73 个函数定义；LyraGame 模块清单 457 个文件，其中 447 个未选中。99 个 unknown 符号位置按原因分为 57 个声明缺失、21 个接收者类型未确定、20 个需要语义证据、1 个非调用引用待检查。这些计数不是全 Lyra 覆盖率或编译诊断。
