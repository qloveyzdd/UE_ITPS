...
本文描述仓库 `tools/` 第一阶段 v1 实现，供代码检查、架构讨论和后续优化使用。v1 是七个 UE 项目检查 CLI 的首个公开契约；此前的其他 Schema 标识属于发布前内部更新步骤，已统一重置为 v1。本文描述的是当前可验证实现，不是最终产品承诺。

## 1. v1 目标与完成状态

v1 已完成 UE 项目进入系统时的第一层确定性检查：
...
第一阶段交付包括七个只读 CLI、共享服务层、统一结果信封、严格输入校验、中英文 CLI 帮助、UTF-8 输出、仓库级 Codex Skill 和契约测试。这些结果未来可以成为 Code Graph 和 Authority Context 的底层证据，但 v1 不会自动生成 Feature Graph、判断功能权威、修改 UE 项目或执行构建。
...
JSON 解析、注册表查询、路径定位和目录分类由程序完成。LLM 负责选择工具、解释结果和提出下一步，不替代可确定的文件系统判断。文件内容哈希只由基线指纹和运行证据工具生成，不进入七个聚焦检查结果。
...
七个聚焦检查工具只读项目、Engine 和注册表，并向标准输出写 JSON。证据生成工具是单独的有写入能力工具。
...
第一阶段 v1 的代码级验收包括：七个 CLI 使用统一结果信封；所有公开 Schema 均为 v1；`validation` 与 `limits` 明确分离事实、诊断和解释边界；输入与项目结构异常按职责失败关闭；33 项单元测试覆盖主要契约。Lyra smoke 结果用于真实工程回归，但静态扫描不替代 UBT、Editor 或运行验证。
...
1. 当前单元测试覆盖统一结果信封、双语 CLI、严格 JSON、描述符压缩、Module 对账、Target 分类、Plugin 定位和路径分类。
2. 尚未完整覆盖 Engine 注册表歧义、Windows junction/symlink、权限不可访问状态和完整 Profile 过滤矩阵。
...
3. 将事实发现、Profile policy 和诊断定级拆开。
...
9. 下一阶段是否先建设测试契约，再继续扩展 Config、Asset 和 Plugin 闭包？
