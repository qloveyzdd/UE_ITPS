...
本文描述仓库 `tools/` 当前可验证实现，供代码检查、架构讨论和后续优化使用。项目级 v1 由七个只读检查 CLI 组成；源码分析 v2 在不修改这些项目级 Schema 的前提下新增四个 CLI，覆盖单个 `.uplugin` 读取、插件模块导航、Build.cs、Target.cs 和模块入口五类聚焦任务。里程碑版本与各工具的独立 Schema 版本分开管理，因此新增工具均从各自的 v1 契约开始。本文描述实际代码，不是最终产品承诺。
...
当前交付包括十一个只读 CLI、共享服务层、统一结果信封、严格输入校验、中英文 CLI 帮助、UTF-8 输出、仓库级 Codex Skill 和 65 项单元测试。这些结果可以成为后续 Code Graph 和 Authority Context 的底层证据，但不会自动生成 Feature Graph、判断功能权威、修改 UE 项目、执行 UBT 规则或证明运行行为。
...
源码规则与模块入口验收包括：四个新 CLI 使用相同结果信封但拥有独立 Schema；`.uplugin`、Build.cs、Target.cs 和模块入口结果互不嵌套；Build.cs 只公开设置变更相关性而不复刻代码结构；模块入口只公开状态结论、传播后的条件和证据位置。当前 65 项单元测试覆盖十一个 CLI 的双语帮助、既有项目契约和新增源码边界。
...
1. 当前 65 项单元测试覆盖统一结果信封、十一个 CLI 的双语帮助、严格 JSON、描述符压缩、Module 对账、Target 分类、Plugin 定位、路径分类、ModuleRules/TargetRules 相关性投影，以及模块入口的状态压缩、条件传播、自由函数回调、默认覆盖、闭合判断和解析入口兼容性。
