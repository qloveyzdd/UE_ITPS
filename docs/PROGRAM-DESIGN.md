...
项目级结果只负责发现和定位，不嵌入后续源码解析结果。调用者先取得 `.uproject` 和直接 Plugin 位置，再显式选择一个 `.uplugin`、Build.cs、Target.cs、模块入口或 `.cpp` 继续查询。v3 对外部 include 只定位来源；是否继续读取该依赖由大模型再次显式选择。每层保留独立 Schema、`validation` 和 `limits`，不创建聚合权威。
