...
| `ue_inspect_module_entry.py` | 压缩单个模块生命周期造成的状态变化 | Build.cs | `ue-itps.module-entry-state.v7` |
...
| `module_entry.py` | 将生命周期、实际绑定回调和可观察默认覆盖压缩为状态模型 |
...
`ue_inspect_module_entry.py` 以所选 Build.cs 的父目录作为模块源码边界，解析模块类、顶层自由函数和三种 `IMPLEMENT_*_MODULE`。调用关系只从 `StartupModule`、`ShutdownModule` 出发，沿同模块辅助函数继续；注册操作中实际绑定到本模块方法或顶层函数的回调会成为额外根。调用点条件和绑定回调成立所需条件会继续传播，条件集合使用 AND 分支、多个路径使用 OR 分支。

公开结果不再复刻方法、调用或赋值结构。`state_models` 只记录状态主体、`default → bound → unbound` 等语义路径、触发根、内部 `via` 名称、有效条件、闭合判断和证据位置；相同行为签名的主体会分组。`conditional_overrides` 记录可观察返回值、引用参数或成员何时从默认状态被覆盖，但省略默认值、覆盖值和 RHS。无法查看被调函数内部状态的外部调用只进入 `unresolved_effects`。
...
- 模块入口只允许生命周期辅助方法和实际绑定回调形成受限可达关系；公开结果只保留状态模型与内部 `via` 名称，不扩展为通用调用图。
