...
| `ue_inspect_module_entry.py` | 读取单个模块的注册、生命周期和委托操作 | Build.cs | `ue-itps.module-entry-source.v6` |
...
- 单模块入口工具以所选 Build.cs 的父目录作为唯一源码边界和绝对 `module_root`；其余路径均相对该根目录，`source_files` 和 `same_class_calls` 不会越过该目录与模块类。
- 委托操作只接受明确列出的 UE Delegate 绑定 API；当前模块类的同名辅助方法只进入 `same_class_calls`，未知 API 不按名称前缀推断。
- 委托注册与注销通过 `related_operations` 区分唯一 handle、复用 handle 候选、callback、object、source 和仅同源候选关系，不把同一委托源上的相反操作统一声明为精确匹配。
- 匹配注册宏是模块类的优先证据；只有缺少匹配注册宏时，才通过 `IModuleInterface` 或 `GameModuleImpl` 继承关系推断候选类。
...
源码规则与模块入口验收包括：四个新 CLI 使用相同结果信封但拥有独立 Schema；`.uplugin`、Build.cs、Target.cs 和模块入口结果互不嵌套；Build.cs 只公开设置变更相关性而不复刻代码结构；位置、控制相关性和未求值表达式按源码事实保留；生命周期只沿辅助方法和实际绑定回调展开。当前 49 项单元测试覆盖十一个 CLI 的双语帮助、既有项目契约和新增源码边界。
...
1. 当前 49 项单元测试覆盖统一结果信封、十一个 CLI 的双语帮助、严格 JSON、描述符压缩、Module 对账、Target 分类、Plugin 定位、路径分类、ModuleRules 相关性投影，以及规则条件、Target 结构化控制、跨文件 Target 候选、控制表达式变更、未求值表达式和生命周期回调边界。
