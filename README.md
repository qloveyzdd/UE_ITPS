...
TargetRules 和 ModuleRules 复用底层词法、位置、控制结构和表达式模型，但使用不同的公开 Schema。TargetRules 保留源码条件，ModuleRules 只公开扁平的生效方式、控制类型和相关输入。所有属性、集合和方法操作均是静态声明证据；工具不会执行构造函数、求值 Target Profile 或生成实际 UBT 构建结果。
...
当前包含 47 项单元测试，覆盖统一结果信封、双语 CLI、严格 JSON、描述符压缩、Module 对账、Target 分类、Plugin 定位、路径分类、ModuleRules 相关性投影，以及独立源码工具的条件、控制表达式变更、未求值表达式和生命周期委托边界。静态检查通过不等于 UE 项目已经构建或运行；Lyra 构建与运行证据仍按 `.planning/evidence/` 中的独立流程管理。
