...
| `ue_inspect_module_entry.py` | `ue-itps.module-entry-source.v6` |
...
当前包含 49 项单元测试，覆盖统一结果信封、双语 CLI、严格 JSON、描述符压缩、Module 对账、Target 分类、Plugin 定位、路径分类、ModuleRules 相关性投影，以及独立源码工具的结构化控制、跨文件 Target 候选、控制表达式变更、未求值表达式和生命周期委托边界。静态检查通过不等于 UE 项目已经构建或运行；Lyra 构建与运行证据仍按 `.planning/evidence/` 中的独立流程管理。
