...
工具第一阶段已经完成，并作为 **v1** 发布：仓库提供七个确定性、只读的 UE 项目检查 CLI，以及负责最小工具路由和结果解释的 `ue-project-inspector` Skill。此前出现的其他 Schema 标识均属于 v1 发布前的内部更新步骤，不作为公开版本保留。
...
当前已完成 UE 5.6.1 本机编译，冻结 9,656 个权威文件的 SHA-256 清单，并归档 Engine/Lyra 来源、Target、Module、Plugin、目录职责、核心 Asset Registry 关系，以及 PIE 启动、Frontend、Session/Travel、四种网络模式、Hard/Seamless 对象存续、失败恢复、Experience、PlayerState、Pawn、ASC、InitState 和输入的静态主链。七个检查 CLI 的公共契约、严格校验、双语帮助、UTF-8 输出和单元测试已经稳定；原始日志不可覆盖复制与 SHA-256 manifest 工具也已就绪。
...
v1 当前包含 33 项单元测试，覆盖统一结果信封、双语 CLI、严格 JSON、描述符压缩、Module 对账、Target 分类、Plugin 定位和路径分类。静态检查通过不等于 UE 项目已经构建或运行；Lyra 构建与运行证据仍按 `.planning/evidence/` 中的独立流程管理。
