...
| `ue_read_plugin_descriptor.py` | `ue-itps.plugin-descriptor.v1` |
...
第一条命令只读取该 `.uplugin` 的字段、Module 声明和直接 Plugin 依赖；第二条命令只对账该插件的 Build.cs 与 `IMPLEMENT_*_MODULE` 入口候选，并返回下一步可使用的绝对路径。两条结果互不嵌套，也不遍历依赖插件。
