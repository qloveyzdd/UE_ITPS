...
当前已完成 UE 5.6.1 本机编译，冻结 9,656 个权威文件的 SHA-256 清单，并归档 Engine/Lyra 来源、Target、Module、Plugin、目录职责、核心 Asset Registry 关系，以及 PIE 启动、Frontend、Session/Travel、四种网络模式、Hard/Seamless 对象存续、失败恢复、Experience、PlayerState、Pawn、ASC、InitState 和输入的静态主链。十二个检查 CLI 的公共契约、严格校验、双语帮助、UTF-8 输出和单元测试已经稳定；原始日志不可覆盖复制与 SHA-256 manifest 工具也已就绪。
...
每个工具维护独立 Schema；新增源码工具从各自的 v1 开始，不改变已有项目级 Schema：
...
| `ue_inspect_plugin_modules.py` | `ue-itps.plugin-modules.v1` |
| `ue_inspect_module_rules.py` | `ue-itps.module-rules-source.v1` |
| `ue_inspect_target_rules.py` | `ue-itps.target-rules-source.v1` |
| `ue_inspect_module_entry.py` | `ue-itps.module-entry-source.v1` |
...
第一条命令读取该 `.uplugin` 的字段、Module 声明和直接 Plugin 依赖，按 UE 5.6.1 枚举与字段类型进行校验，并在插件 `Source`、`Platforms` 下递归对账 Module 对应的 Build.cs。Build.cs 通过文件名匹配，`Source/<Name>/<Name>.Build.cs` 只标记为传统位置，不是硬性要求。第二条命令复用相同对账结果并定位 `IMPLEMENT_*_MODULE` 入口候选。两条结果互不嵌套，也不遍历依赖插件。
...
Build.cs 工具输出 `ModuleRules` 类、构造函数、同文件辅助方法、直接调用关系、规则操作、条件上下文、位置和未求值表达式。模块入口工具以 Build.cs 父目录作为单模块边界，只跟踪 `StartupModule`、`ShutdownModule`、其同文件辅助方法以及实际绑定的回调，不生成通用 C++ 类图或完整调用图。
...
TargetRules 和 ModuleRules 复用底层词法、位置、条件和表达式模型，但使用不同的公开 Schema。所有属性、集合和方法操作均是静态声明证据；工具不会执行构造函数、求值 Target Profile 或生成实际 UBT 构建结果。
...
- 十二个聚焦检查 CLI 默认只读，并将 JSON 写到标准输出。
- 十二个检查 CLI 不计算或输出文件内容哈希；基线指纹工具保持独立职责。
...
当前包含 38 项单元测试，覆盖统一结果信封、双语 CLI、严格 JSON、描述符压缩、Module 对账、Target 分类、Plugin 定位、路径分类，以及独立源码工具的条件、未求值表达式、插件模块导航和生命周期委托边界。静态检查通过不等于 UE 项目已经构建或运行；Lyra 构建与运行证据仍按 `.planning/evidence/` 中的独立流程管理。
