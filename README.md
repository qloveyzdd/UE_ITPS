...
| `ue_inspect_module_entry.py` | `ue-itps.module-entry-state.v7` |
...
模块入口 v7 以 Build.cs 父目录作为单模块边界，从 `StartupModule`、`ShutdownModule` 和实际绑定的同模块回调建立受限函数图。内部辅助函数只作为 `via` 名称，调用点和回调绑定条件会传播到状态事件。公开结果仅包含 `state_models`、`conditional_overrides`、`unresolved_effects` 和源码证据；方法签名、原始调用、赋值 RHS 与具体覆盖值不会输出。`default` 表示该模块首次介入前的状态，不代表工具证明了 UE 对象的构造默认值。
