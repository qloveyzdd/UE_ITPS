...
`ULyraSystemStatics::PlayNextGame` 显式向 LastURL 添加 `SeamlessTravel`，使用 Relative ServerTravel 保持客户端连接。Engine 会保留 PlayerState 等 Seamless Actor，并走 `FSeamlessTravelHandler`，不能复用上面的 Hard Travel 对象表。
...
- Hard Travel 改为 Seamless Travel，改变 PlayerState/ASC 等对象存续；
- Join 成功回调时机改变；
