...
> **历史调查**：正文的“当前”和“下一步”均指原调查日期，不代表现有版本的验证结果或待执行任务。版本与状态见[基线索引](BASELINE.md)，工具实现见[当前项目状态](../PROJECT.md)。
...
| 模式 | 关键分叉 |
|---|---|
| Standalone | Offline Host URL 无 `listen`；同进程执行 Client + Server 逻辑，无网络连接握手 |
| Listen Server | LAN/Online Host URL 有 `listen`；本地 PC 走 PreClientTravel，远端 PC 收到 ClientTravel |
| Dedicated Server | 默认前端图先做服务登录和 UserFacingExperience 选择；无 LocalPlayer/LoadingScreen，只加载 Server Bundle |
| Client | Join 成功后绝对 ClientTravel；`Browse → PendingNetGame → LoadMap`；`CurrentExperience` 复制后只加载 Client Bundle |

Seamless Travel 也不是 Hard Travel 对象表的简单反面：服务端目标 GameMode 默认创建新 PC/PS，并通过 `CopyProperties` 迁移有限数据。Lyra 的 PlayerState 自定义 Copy 只有基类调用和 `Copy stats` TODO，因此 ASC、PawnData、Team/Squad、Attribute/Effect 必须作为独立重建或迁移证据。

详细 Frontend 链见 `.planning/codebase/TRAVEL.md`；Target/NetMode、四模式时序、Seamless 对象存续与失败恢复见 `.planning/codebase/NETWORK-MODES.md`。
...
- Target 与 Runtime NetMode 的分离，以及 URL/NetDriver 对 Standalone、Listen、Client 的推导规则。
- Offline 不加 `listen`、LAN/Online 添加 `listen` 的 Host URL 规则。
- Dedicated Server 在默认前端图登录、选择 UserFacingExperience、Host 后旅行的静态顺序。
- Client `Join success → ClientTravel → Browse → PendingNetGame → LoadMap` 的 Engine/CommonSession 静态顺序。
- Seamless Travel 默认创建新 PC/PS、Lyra 仅复制基类 PlayerState 字段并重新等待目标 Experience 的静态语义。
- Loading Screen 对 LoadMap、TravelURL、PendingNetGame、Seamless、Experience 和 Frontend Flow 的屏障规则。
...
- Seamless 前后 PC、PS、ASC、Pawn、GameState 和 ExperienceManager 的对象身份与业务数据结果。
- Network/Travel Failure 后错误 UI、Session/User 状态与 Loading Screen 的完整恢复。
...
3. 分别运行 Standalone、Listen Server、Dedicated Server、Client，验证已建立的静态时序差异，避免用单机 PIE 推断网络权威。
4. 注入 Join 后连接失败与 Seamless Travel，验证错误恢复和对象迁移，而不是只跑成功 Hard Travel。
