...
- V4：复用项目工具池探针，将项目级 C++ 符号与关系写入 SQLite，并提供查询、并行扫描和可恢复缓存。
...
V4 数据图谱不位于项目工具池之上，也不直接驱动工具。最终系统具有两个平级资源池：
...
项目工具池当前已提供统一注册和能力发现，并保持各工具稳定的职责边界与输出契约。未来产品级能力池可以在此基础上补充权限、工作区和验证元数据。
...
其中工具标识、入口、输入和能力元数据已进入当前注册表；权限、工作区和产品级验证仍属于后续设计。
